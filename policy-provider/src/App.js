...
function App() {
  ...
  const handleSetOracleAddress = async (e) => {
    e.preventDefault();
    ...
    await ins.methods.setOracleAddress(ORACLE_ADDRESS).send({ from: accounts[0] }).once('recipt', (recipt) => {
      //setLoading(false)
      console.log(recipt)
    })

    setOracleAddr(ORACLE_ADDRESS)
  ...
  const handleDeposit = async (e) => {
    ...
    await ins.methods.depositPolicyAmount().send({
      from: accounts[0],
      value: '100'
    })
  ...
  const handleEndPolicy = async (e) => {
    ...
    await ins.methods.endPolicy().call()
  ...
  return (
      <div className="App">
        ...
        <button onClick={handleDeposit}>
          <input type="submit" value="Deposit Coverage (100 wei)" />
        </button>

        <button onClick={handleSetOracleAddress}>
          <input type="submit" value="Set Oracle..." />
        </button>
        <button onClick={handleEndPolicy}>
          <input type="submit" value="End Policy" />
        </button>
      </div>
  );
}
